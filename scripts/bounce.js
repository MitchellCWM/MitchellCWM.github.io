...
function round(n1) {
    if (n1 < 0) {
        return Math.floor(n1) === 0 ? -.0001 : Math.floor(n1);
    }
    else {
        return Math.ceil(n1) === 0 ? .0001 : Math.ceil(n1);
    }
}
let balls = [];
        ...
        for (const ball of balls) {
            if (!(this === ball)) {
                if (this.isCollision(ball)) {
                    ...
                    ball.velx = (ball.velx * -1) * (random(55, 15) / 10);
                    this.velx = (totalVelX) / round(ball.velx);

                    let totalVelY = this.vely + ball.vely;
                    ball.vely = (ball.vely * -1) * (random(5, 15) / 10);
                    this.vely = (totalVelY) / round(ball.vely);
                    while (this.isCollision(ball)) {
                        this.update();
                        this.update();
                    }
                }
...
balls[0] = new ball(-2.5, 10, 10, 'blue');
balls[1] = new ball(-10, .1, 10, 'green');
balls[2] = new ball(2.5, 2.5, 10, 'yellow');
balls[3] = new ball(-5.0, 5.0, 10, 'white');
balls[4] = new ball(6.0, 3.0, 10, 'red');
balls[5] = new ball(-2.5, 1.0, 10, 'blue');
balls[6] = new ball(-10, .1, 10, 'green');
balls[7] = new ball(2.5, 2.5, 10, 'yellow');
balls[8] = new ball(-5.0, 5.0, 10, 'white');
balls[9] = new ball(6.0, 3.0, 10, 'red');
balls[10] = new ball(-2.5, 1.0, 10, 'blue');
balls[11] = new ball(-10, .1, 10, 'green');
balls[12] = new ball(2.5, 2.5, 10, 'yellow');
balls[13] = new ball(-5.0, 5.0, 10, 'white');
balls[14] = new ball(6.0, 3.0, 10, 'red');
